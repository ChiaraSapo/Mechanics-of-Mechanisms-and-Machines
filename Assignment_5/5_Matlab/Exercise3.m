...
% It considers the long link as the fixed one
...
for alfa= 0:0.1:4*pi
    ...
    b = 4; % First digit of Matricola 
    a = 1/(0.1+(0.012*9)); % n = 9;

    %Calculating relations between the angles
    x = sqrt((b^2)+(a^2) -(2*b*a*(cos(alfa)))); %Cosine theorem: x^2=a^2+b^2+2abcos(alpha)
    theta = asin((b*sin(alfa))/x); %Sine theorem: sin(alfa)/x=sin(theta)/a
    beta = (pi-2*theta-alfa); %Sum of internal angles of a triangle

    %Design of various points of the mechanism
    P1 = [0 0]; 
    P2 = [a 0];
    P3 = [b*cos(beta) -b*sin(beta)];
    P4 = [a-b*cos(alfa) b*sin(alfa)];
    ...
    P1 = [-b*cos(betaFixed) b*sin(betaFixed)]; 
    P2 = [a b]; 
    P3 = [b*cos(beta) -b*sin(beta)];
    P4 = [a-b*cos(beta) -b*sin(alfa)];
